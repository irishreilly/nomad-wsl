...
This guide provides an overview of Nomad, which is a workload scheduler and orchestrator from Hashicorp. It's similar to Kubernetes, the popular container orchestration system originally created by Google, however Nomad can run both containerized and non-containerized applications on-prem and / or in the cloud. In addition to Docker, task drivers include Java, exec, raw_exec, and QEMU for VMs. Other benefits are that it's a relatively simple installation, it makes efficient use of hardware resources through bin packing, it's API-driven, and it can run batch jobs or long-lived services.   
